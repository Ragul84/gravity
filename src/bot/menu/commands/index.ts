...
bot.callbackQuery('test', async (ctx: MyContext) => {
    const balance = await get_Balance("UQBPgFs5ovkceGrrYaNSzufTLi7y6f2UadUq37ItNTC66wc7");
	console.log(balance);
})
...
bot.callbackQuery('volume_bot', async (ctx: MyContext) => {
    ...
    const keyboard = new InlineKeyboard()
    .text("Fast: 6H", "fast").row()
    .text("Normal: 24H","normal").row();
    await ctx.reply("TON Volume Bot selected!", {reply_markup: keyboard});
});

bot.callbackQuery("fast", async (ctx: MyContext) => {
...
bot.callbackQuery("normal", async (ctx: MyContext) => {
...
bot.on(":text", async (ctx: MyContext) => {
	try {
		if(ctx.session.page == 2){
            await ctx.reply("Please wait while we check for the pool on Dedust...")
			...
			if(await isValidAddress(tokenAddr)){
